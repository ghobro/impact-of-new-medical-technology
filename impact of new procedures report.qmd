...
The following SQL script shows the way in which the data are extracted for elective admissions and outpatient attendances. `new_codes` refers to the 666 codes that were identified as being added since OPCS-4.3 in 2006 (having excluded chapters Y and Z).

``` sql
select 
    'elective inpatients' as pod,
    year([OPDATE]) as [year], 
    [OPCODE] as [primary_procedure], 
    count(*) as [n] 
from 
...
where 
    [OPCODE] in new_codes
    and year([OPDATE]) between 2000 and 2019
    and OPORDER = 1
...
where
    [OPCODE] in new_codes
    and year([APPTDATE]) between 2000 and 2019
    and [OPORDER] = 1
group by 
    year([APPTDATE]),
    [OPCODE] ;
...
new_procedure_counts_full <- new_procedure_counts_full |>
  filter(version_introduced != c("4.9", "4.10"))
...
hes_activity_data <- read.csv(
  here("data", "Hospital activity data (HES).csv"))

# filter to opa and elective and year up to 2019
hes_opa_and_elective <- hes_activity_data |>
  filter(pod %in% c("IpElec", "opAtt"),
         yr <= 2019)

# re-group the data
hes_opa_and_elective <- hes_opa_and_elective |>
  summarise(activity = sum(activity), .by = c(yr, pod)) |> 
  #rename the pods
  mutate(pod = if_else(pod=="IpElec", "elective inpatients", "outpatient attendances"))
...
ggplot() +
  ...
  geom_line(data = hes_opa_and_elective, aes(x=yr,y=activity)) +
